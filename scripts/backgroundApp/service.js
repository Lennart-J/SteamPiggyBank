...
.service('requestService', function($http, $q, util) {
  ...
  this.getAppItemDetails = function(appId, packageId) {
    var categories = [], description, $data, defer = $q.defer(), app, userTags = [];
    ...
    if (appId) {
      $http.get('http://store.steampowered.com/apphover/' + appId)
        .success(function(data){
          ...
          defer.resolve({'appId' : appId,'categories':categories, 'userTags':userTags, 'description':description});
      ...
      $http.get('http://store.steampowered.com/subhover/' + packageId)
        .success(function(data){
          ...
          userTags = getUserTags($data);
          description = getDescription($data);
          defer.resolve({'packageId' : packageId, 'categories':categories, 'userTags':userTags, 'description':description});
  ...
  var parseDOMElementList = function(list) {
    ...
    function getDiscount(element) {
      return  parseInt(element.find('.search_discount span').text().replace(/\-|\%/g, ''));
    }
